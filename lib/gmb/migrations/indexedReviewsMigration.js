...
function isAllowedReviewKey({ key, tenantId }) {
  return typeof key === "string" && key.startsWith(`gmb:${tenantId}:review:`);
}

function createResult({ tenantId, date, offsetIndexes, limitIndexes, dryRun, discovery }) {
  return {
    tenant_id: tenantId,
    date,
    dry_run: dryRun,
...
function addError(result, key, error) {
  result.failed += 1;
  if (result.errors.length < 20) result.errors.push({ key, error: error.message });
}
...
async function discoverIndexes({ tenantId, date, limitIndexes }) {
  const pattern = date
    ? `gmb:${tenantId}:index:${date}:place:*:review_keys`
    : `gmb:${tenantId}:index:*:place:*:review_keys`;
  const scan = await scanAllKeys({ pattern, count: 1000, maxKeys: limitIndexes });
...
async function processReviewKey({ reviewKey, tenantId, dryRun, result }) {
  ...
  const review = parseRedisJson(await redis(["GET", reviewKey]), "review");
  ...
  if (!dryRun) await upsertReview({ review, tenantId });
  result.inserted_or_updated += 1;
}

async function processIndex({ indexKey, tenantId, dryRun, result }) {
  const reviewKeys = parseRedisJson(await redis(["GET", indexKey]), "review_keys");
  if (!Array.isArray(reviewKeys)) throw new Error("review_keys_not_array");
...
export async function migrateIndexedReviews({ tenantId, date, offsetIndexes, limitIndexes, dryRun }) {
  const discovery = await discoverIndexes({ tenantId, date, limitIndexes: 5000 });
  const batch = discovery.keys.slice(offsetIndexes, offsetIndexes + limitIndexes);
  const result = createResult({ tenantId, date, offsetIndexes, limitIndexes, dryRun, discovery });
