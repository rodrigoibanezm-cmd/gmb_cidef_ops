...
async function resolveSnapshotDate({ tenantId, date }) {
  const rows = await dbQuery(
    `SELECT COALESCE($2::date, MAX(captured_date)::date) AS snapshot_date
...
async function assertCompleteCapture({ tenantId, date }) {
  const rows = await dbQuery(
    `SELECT
       (SELECT COUNT(*)::int FROM places WHERE tenant_id = $1 AND COALESCE(status, 'keep') = 'keep') AS total_places,
       (SELECT COUNT(DISTINCT place_id)::int FROM place_daily_metrics WHERE tenant_id = $1 AND captured_date = $2) AS captured_places`,
    [tenantId, date]
  );

  const total = rows[0]?.total_places ?? 0;
  const captured = rows[0]?.captured_places ?? 0;

  if (total === 0 || captured < total) {
    const error = new Error('capture_incomplete');
    error.code = 'capture_incomplete';
    error.total_places = total;
    error.captured_places = captured;
    throw error;
  }
}
...
export async function rebuildDashboardSnapshot({ tenantId, date = null }) {
  ...
  const snapshotDate = await resolveSnapshotDate({ tenantId, date });

  if (!snapshotDate) {
    const error = new Error('snapshot_date_not_found');
    error.code = 'snapshot_date_not_found';
    throw error;
  }

  await assertCompleteCapture({ tenantId, date: snapshotDate });
  await ensureDashboardSnapshotsTable();

  const rows = await dbQuery(
    `
    WITH params AS (
      SELECT $1::text AS tenant_id, $2::text AS tenant_name, $3::date AS snapshot_date
    ),
    base AS (
      SELECT m.tenant_id, m.place_id, m.captured_date, m.rating, m.review_count,
             p.name, p.brand, p.operator, p.store_role, p.normalized_location, p.ownership_group
      FROM place_daily_metrics m
      JOIN places p ON p.tenant_id = m.tenant_id AND p.place_id = m.place_id
      JOIN params x ON x.tenant_id = m.tenant_id AND x.snapshot_date = m.captured_date
      WHERE m.rating IS NOT NULL AND m.review_count > 0
    ),
    own_base AS (
      SELECT * FROM base WHERE ownership_group = 'own'
    ),
    own_confident AS (
      SELECT * FROM own_base WHERE review_count >= $4
    ),
    ...
    red_flags AS (
      ...
      FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY rating ASC, review_count DESC) AS rank,
               place_id, name, brand, operator, store_role, normalized_location AS location,
               rating, 0 AS delta, review_count,
    ...
    top AS (
      SELECT jsonb_agg(to_jsonb(x)) AS data
      FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY rating DESC, review_count DESC) AS rank,
               place_id, name, brand, operator, store_role, normalized_location AS location,
               rating, 0 AS delta, review_count,
               'low' AS severity,
    ...
    own_risk AS (
      SELECT o.place_id AS own_place_id,
             o.name AS own_name,
             o.brand,
             o.operator,
             o.store_role,
      ...
      JOIN competitor_leaders c ON c.normalized_location = o.normalized_location
      WHERE o.rating < c.leader_rating
      ORDER BY gap_vs_leader ASC, o.review_count DESC
      LIMIT 10
    ),
    own_risk_ranked AS (
      SELECT ROW_NUMBER() OVER (ORDER BY gap_vs_leader ASC, own_review_count DESC) AS local_rank,
    ...
    dominant_competitor AS (
      SELECT CASE WHEN $6 = 'operator' THEN leader_operator ELSE leader_brand END AS competitor,
             COUNT(*)::int AS cnt
      FROM own_risk
    ...
    INSERT INTO dashboard_snapshots (tenant_id, snapshot_date, view, payload, source, updated_at)
    SELECT x.tenant_id,
           x.snapshot_date,
           'full',
           jsonb_build_object(
             ...
             'updated_at', NOW(),
             'period', jsonb_build_object('current_date', x.snapshot_date, 'comparison_label', 'vs. período anterior'),
             ...
             'executive_summary', jsonb_build_object(
               ...
               'desktop_hint', 'La red propia mantiene una nota promedio de ' || ROUND(k.average_rating::numeric, 2) || ', con ' || k.critical_stores::text || CASE WHEN $7 LIKE 'puntos%' THEN ' puntos bajo el umbral reputacional.' ELSE ' tiendas bajo el umbral reputacional.' END,
               'mobile_hint', k.critical_stores::text || CASE WHEN $7 LIKE 'puntos%' THEN ' puntos de la red requieren atención reputacional.' ELSE ' tiendas propias requieren atención reputacional.' END
             ),
             'competitive_summary', jsonb_build_object(
               ...
               'headline', CASE
                 ...
                 ELSE x.tenant_name || ' ' || $8 || ' ' || cs.risk_count::text || ' ' || $7 || '. ' || INITCAP(REPLACE(cs.dominant_competitor, '_', ' ')) || ' lidera en ' || cs.dominant_competitor_count::text || ' ' || $9 || '.'
               END
             ),
             'mobile_priority', jsonb_build_object(
               'headline', cs.risk_count::text || ' ' || $7 || ' con riesgo competitivo local.',
             ...
             'movements', jsonb_build_object('up', '[]'::jsonb, 'down', '[]'::jsonb),
             'rankings', jsonb_build_object('top', COALESCE((SELECT data FROM top), '[]'::jsonb), 'bottom', COALESCE((SELECT data FROM bottom), '[]'::jsonb))
           ),
           'neon_materialized',
           NOW()
    FROM params x
    ...
    RETURNING tenant_id, snapshot_date, view, source, payload->'competitive_summary' AS competitive_summary, payload->'kpis' AS kpis
    `,
    [
      tenantId,
      config.tenantName,
