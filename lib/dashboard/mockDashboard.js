...
export function getMockDashboard(tenant_id) {
  ...
  const redFlags = [
    store(tenant_id, cfg.locations[0], 3.6, -0.22, {
      review_count: 1140,
      severity: "high",
      reason: "Alta caída + alto volumen de reviews"
    }),
    store(tenant_id, cfg.locations[1], 3.8, -0.15, {
      review_count: 860,
      severity: "high",
      reason: "Bajo 4.0 sostenido"
    }),
    store(tenant_id, cfg.locations[2], 3.9, -0.11, {
      review_count: 640,
      severity: "medium",
      reason: "Deterioro reciente con volumen relevante"
    })
  ];
  ...
  return {
    ...
    executive_summary: {
      ...
      desktop_hint: `Deterioro concentrado en ${cfg.locations[0]} y ${cfg.locations[1]}.`,
      mobile_hint: `${cfg.locations[0]} y ${cfg.locations[1]} requieren atención.`
    },
    mobile_priority: {
      headline: `La caída se concentra en ${cfg.locations[0]} y ${cfg.locations[1]}.`,
    ...
    rankings: {
      ...
      bottom: [
        ...
        redFlags[1],
        redFlags[2],
        store(tenant_id, "San Bernardo", 4.0, -0.07, { review_count: 450, severity: "medium", reason: "Cerca de umbral crítico" }),
        store(tenant_id, "Osorno", 4.05, -0.05, { review_count: 260, severity: "medium", reason: "Deterioro moderado" })
      ].map((item, index) => ({ rank: index + 1, ...item }))
    }
  };
}
